...
void luaK_nil (FuncState *fs, int from, int n) {
	...
	if (fs->pc > fs->lasttarget) {  /* no jumps to current position? */ //看不懂
		if (fs->pc == 0) {  /* function start? */ //开始的loadnil忽略
			...
			previous = &fs->f->code[fs->pc-1]; //连续两条loadnil时，判断loadnil的区间是否可以合并, 可以的话，缩成一条指令
...
//加到jpc的list, 下一次编码时，会按照那时的pc进行fixjmp修正offset
...
//l1 l2都是jmp指令，至少有个offset, 将每个concat的l2形成一个链表
...
/*
 * 新加入一个常量， 将他在fs->h中的key设置为fs->nk。
 * 下次如果再加入这个常量，可以通过key找到原先的fs->nk
 */
static int addk (FuncState *fs, TValue *k, TValue *v) {
	// v 放在f->k数组中， fs->h保存key到数组的索引
	lua_State *L = fs->L;
...
int luaK_exp2anyreg (FuncState *fs, expdesc *e) {
	luaK_dischargevars(fs, e);
	if (e->k == VNONRELOC) { //local -> v non reloc
